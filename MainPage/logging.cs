...
namespace Catan10
{
    ...
    public class Log : List<LogEntry>
    {
        ...
        /// <summary>
        ///    we keep track of the last rectord we wrote to disk in _lastLogRecordWritten
        ///    start there in the list of log records in the list and the write all the subsequent
        ///    records to disk.  remember how far we made it.
        ///    mark the records as persisted by setting the flag in logRecord
        /// </summary>
        /// <returns></returns>
        public async Task WriteUnwrittenLinesToDisk()
        {
            try
            {
                Debug.Assert(_file != null, "_file is null - Init must have been skipped");
                ...
                for (int i = _lastLogRecordWritten; i < count; i++)
                {
                    ...
                    if (!le.Persisted)
                    {
                        ...
                        //
                        //  you might think that this should be set *before* AppentTextAsync
                        //  but you would be wrong.  if there is ever going to be an exception
                        //  it will be when you write to disk.  by putting it after the write,
                        //  we will eventually get this record to disk. the flag is not written
                        //  in the log
    ...
    static readonly private string[] _serializeProperties = new string[] { "LogLineIndex", "LogType", "Undone", "PlayerDataString", "GameState", "Action", "Number", "IndexOfUndoneAction", "StopProcessingUndo", "TagAsString", "MemberName", "LineNumber" }; // "Path" removed
                                                                                                                                                                                                                                                               /// <summary>
